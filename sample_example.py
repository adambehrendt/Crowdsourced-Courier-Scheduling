...
def example(inhomogeneous=False):
	...
	if not inhomogeneous:
		X = genfromtxt('data_homogeneous.csv', delimiter=',')
	else:
		X = genfromtxt('data_inhomogeneous.csv', delimiter=',')
	...
	for k in range(K):
		order_list = utils.gen_random_order_list(N_rv, 45, pickup_histogram, S, distance_rv, origin_rv, origin_list, num_known, placement_is_pickup=False)
		...
		if Lambda == 0:
			...
			if not inhomogeneous:
				ad_hoc_set[k] = utils.gen_poisson_arrivals(const_rate, T+S, start=0)
			else:
				ad_hoc_set[k] = utils.gen_inhomogeneous_arrivals(const_rate, T+S, num_periods)
