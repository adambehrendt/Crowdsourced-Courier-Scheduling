...
def gen_random_order_list(N_rv, placement_rv, pickup_histogram, window_rv, distance_rv, origin_rv, origin_list, num_known, placement_is_pickup=False):
    ...
    for i in range(N):
        if placement_is_pickup is True:
            order_list[i] = Order(placement_times[i], placement_times[i], placement_times[i]+window_lengths[i], origin_vals[i], origin_locs[i], delivery_locs[i], i)
        else:
